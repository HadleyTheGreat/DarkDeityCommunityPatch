...
$SteamAppInstallPath = Get-SteamGamePath -AppID $AppID
if (-not $SteamAppInstallPath) {
    Throw "Could not find game installation folder path."
}
$ReplaceFilePath = Join-Path $SteamAppInstallPath $FileToReplace
if ([System.IO.File]::Exists($ReplaceFilePath)) {
    Remove-Item -Path $ReplaceFilePath -Force
}

Write-Host "Launching the Steam file integrity check for app id $AppID"

# Launch the Steam file integrity check to get Steam to replace the deleted file
...
WaitFor-FileExists $ReplaceFilePath
