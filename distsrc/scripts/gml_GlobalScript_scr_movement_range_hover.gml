function scr_movement_range_hover(arg0, arg1, arg2)
{
    scr_wipe_nodes();
    var start = arg0;
    var range = arg1;
    ...
    if (actions > 0 && instance_exists(obj_cursor) && obj_cursor.HoverNode != -4 && obj_cursor.HoverNode.occupant != -4 && obj_cursor.HoverNode.occupant != undefined && instance_exists(obj_cursor.HoverNode.occupant))
    {
        var open = ds_priority_create();
        var closed = ds_list_create();
        
        if (ds_exists(open, ds_type_priority))
            ds_priority_add(open, start, start.G);
        
        var current;
        
        if (ds_exists(open, ds_type_priority) && ds_exists(closed, ds_type_list))
        {
            while (ds_priority_size(open) > 0)
            {
                ...
                if (current != -4 && current != 0 && current != undefined)
                {
                    if (instance_exists(current) && scr_debug_assert_instance_type(current, 170, "movement_range_hover:current"))
                    {
                        if (ds_exists(current.neighbors, ds_type_list))
                        {
                            for (ii = 0; ii < ds_list_size(current.neighbors); ii += 1)
                            {
                                ...
                                if (neighbor != -4 && neighbor != 0 && neighbor != undefined)
                                {
                                    if (instance_exists(neighbor) && scr_debug_assert_instance_type(neighbor, 170, "movement_range_hover:neighbor"))
                                    {
                                        if (ds_list_find_index(closed, neighbor) < 0 && neighbor.passable && (neighbor.occupant == -4 || neighbor.occupant == undefined || !instance_exists(neighbor.occupant) || neighbor.occupant.army == HoverNode.occupant.army) && (neighbor.cost + current.G) <= range)
                                        {
        ...
        if (ds_exists(closed, ds_type_list))
        {
            for (ii = 0; ii < ds_list_size(closed); ii += 1)
            {
                ...
                if (current.occupant != -4 && current.G <= range)
                {
                    if (current.occupant != undefined)
                    {
                        if (instance_exists(current.occupant) && instance_exists(obj_cursor.HoverNode) && instance_exists(obj_cursor.HoverNode.occupant))
                        {
                ...
                if (HoverNode.occupant.onlyranged == false)
                    scr_set_attackdisplay_range1(current);
                
                if (HoverNode.occupant.attacktype == "ranged")
                    scr_set_attackdisplay_range2(current);
                
                if (HoverNode.occupant.class == "Sniper" && HoverNode.occupant.army == 20)
