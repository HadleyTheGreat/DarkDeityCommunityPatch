function scr_movement_range_ai_dragon(arg0, arg1, arg2)
{
    scr_wipe_nodes();
    var start = arg0;
    var range = arg1;
    ...
    while (ds_priority_size(open) > 0)
    {
        ...
        {
            if (instance_exists(current) && scr_debug_assert_instance_type(current, 170, "scr_movement_range_ai_dragon:current"))
            {
                if (ds_exists(current.neighbors, ds_type_list))
                {
                    for (ii = 0; ii < ds_list_size(current.neighbors); ii += 1)
                    {
                        ...
                        {
                            if (instance_exists(neighbor) && scr_debug_assert_instance_type(neighbor, 170, "scr_movement_range_ai_dragon:neighbor"))
                            {
