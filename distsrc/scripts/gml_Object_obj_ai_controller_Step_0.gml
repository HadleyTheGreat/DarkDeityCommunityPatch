...
switch (state)
{
                        ...
                        if (ii == i && state == "deciding action")
                        {
                            if (rusher)
                            {
                                ...
                                with (obj_actor)
                                {
                                    if (army == 20)
                                    {
                                        if (other.targetselect == -4 || other.targetselect == 0)
                                        {
                                            other.targetselect = id;
                                        }
                                        else if (instance_exists(other.targetselect))
                                        {
                                            if ((abs(GridX - other.GridX) + abs(GridY - other.GridY)) < (abs(other.targetselect.GridX - other.GridX) + abs(other.targetselect.GridY - other.GridY)))
                                            {
                                                if (room == room_l20)
                                                {
                    ...
                    for (ii = 0; ii <= 15; ii += 1)
                    {
                        if (ds_grid_get(healtargets, 2, ii) > 0)
                        {
                            ...
                            if (distance_to_object(healtarget) <= ((move * 32) + 64) && healtarget.GridX > 1 && healtarget.GridY > 1)
                            {
                                ...
                                {
                                    targetmove = ds_list_find_value(healtarget.nodeimon.neighbors, 0);
                                    wanttoheal = true;
                                    state = "move to healtarget";
                                }
    ...
    case "check for double":
        ...
        else if (attacktarget.eff_spd > (eff_spd + 4) && attacktarget.class != "Champion" && (name != "Empyrean" || !instance_exists(obj_enemy_delian_magician)))
        {
            ...
            if (tempHP <= 0)
            {
                ...
                if (obj_game.enemyturnskip == false)
                {
                    with (instance_create(0, 0, obj_animator_4))
                    {
                        ...
                        xptogive = min(round(other.attacktarget.xptogive * (global.randomXPmodifier / 100)), 100);
                        aigivingxp = other.id;
                    }
                }
                else
                ...
                if (droppable_item < 494)
                {
                    ...
                    else
                    {
                        for (ii = 0; ii <= 499; ii += 1)
                        {
                            ...
                            {
                                ds_grid_set(obj_storage_holder.item_storage, 0, ii, droppable_item);
                                break;
                            }
                        }
